...
const installHelmChart = (path: string) => {
  cy.exec(
    `cd ../../console-plugin-template && \
      ${path} upgrade \
        -i ${PLUGIN_TEMPLATE_NAME} \
        charts/openshift-console-plugin \
        -n ${PLUGIN_TEMPLATE_NAME} \
        --create-namespace \
        --set plugin.image=${PLUGIN_TEMPLATE_PULL_SPEC}`,
    {
...
const deleteHelmChart = (path: string) => {
  cy.exec(
    `cd ../../console-plugin-template && \
      ${path} uninstall ${PLUGIN_TEMPLATE_NAME} \
        -n ${PLUGIN_TEMPLATE_NAME} && \
      oc delete namespaces ${PLUGIN_TEMPLATE_NAME}`,
    {
      failOnNonZeroExit: false,
    },
  ).then((result) => {
